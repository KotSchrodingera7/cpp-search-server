...
public:
    ...
    vector<Document> FindTopDocuments(const string& raw_query,
        ...
        if( !CheckQuery(raw_query) ) {
            throw invalid_argument("Wrong resuest ");
        }
...
private:
    ...
    bool CheckQuery(const string &text) const {
        ...
        for (const string& word : SplitIntoWords(text)) {
            if( word.size() == 1 && word.at(0) == '-') {
                return false;
            }
            const QueryWord query_word = ParseQueryWord(word);
            if (!query_word.is_stop) {
                if (query_word.is_minus) {
                    if( query_word.data.at(0) == '-') {
                        return false;
                    }
                    query.minus_words.insert(query_word.data);
                } else {
...
int main() {
    try {
        ...
        for (const Document& document : search_server.FindTopDocuments("пушистый кот выразительные"s)) {
            PrintDocument(document);
        }
    } catch(...)
    {
        cout << "Exception was caught"s << endl;
    }
    return 0;
}